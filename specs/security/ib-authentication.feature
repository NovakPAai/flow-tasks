...
Feature: Идентификация, аутентификация и авторизация пользователей
  ...
  # ─── 7. Rate-limit по email (gap-14) ─────────────────────────────────────

  # Req ИБ §1.6; gap-14-rate-limit-ip-bypass
  Scenario: Ротация X-Forwarded-For не обходит rate-limit на /login
    Given атакующий знает email жертвы "victim@flowtask.dev"
    When выполнено 10 POST /api/auth/login с разными X-Forwarded-For: 10.0.0.N
    Then 11-й запрос возвращает 429 Too Many Requests
    And тело содержит {"error":"Too many requests"}

  Scenario: Rate-limit применяется к /register и /forgot-password
    When выполнено 10 POST /api/auth/register {"email":"flood@flowtask.dev",...}
    Then 11-й запрос возвращает 429

  # ─── 8. Отсутствие email enumeration (gap-15) ─────────────────────────────

  # Req ИБ §1.2; gap-15-email-enumeration
  Scenario: POST /register возвращает одинаковый ответ для всех статусов email
    Given существует пользователь "existing@flowtask.dev"
    And есть PENDING заявка "pending@flowtask.dev"
    When три запроса POST /api/auth/register для existing/pending/new email
    Then все три ответа имеют статус 200
    And body.message идентично для всех трёх ответов

  # ─── 9. Синхронизация системного времени ──────────────────────────────────
