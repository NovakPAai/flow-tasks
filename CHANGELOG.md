...
## [v1.7.0] — 2026-05-18 — Корзина, быстрая правка срока, polish

### Что нового

- **Быстрая правка срока задачи** (#167): клик по дате прямо на карточке, в списке или в «Моих задачах» открывает календарь. TaskDrawer открывать не нужно.
- **Корзина воркспейсов** (#157): удалённый воркспейс попадает в корзину на 10 рабочих дней. Восстановление и безвозвратное удаление — в меню профиля → «Корзина». Безвозвратное удаление требует ввести имя воркспейса.
- **Свернуть/развернуть всё в Дорожной карте** (#156): кнопка в тулбаре и клавиша `E` скрывают или раскрывают все подзадачи на доске одним действием.
- **«Настройки пространства» в меню аватарки** (#159): доступны из любой страницы, не только из контекста воркспейса.
- **Подсказки требований к паролю на регистрации** (#162): живой чек-лист под полем — длина, заглавная буква, цифра.

### Исправления

- **Дорожная карта — milestone-маркер и UX-полировка** (#158, #160): задачи только со сроком теперь ромб, а не тонкая чёрточка. Подкручены легенда, touch-тултип, скелетоны, клавиатурная навигация.
- **Стандарты разработки + OpenAPI из Zod** (#161): Claude-команды для команды, автогенерация OpenAPI из Zod-схем.
- **`.env` override** (#166): значения из `.env` теперь всегда перекрывают окружение процесса. Закрывает root-cause инцидента с устаревшим `GITHUB_ISSUES_TOKEN`.

### Миграция БД

Миграция `20260518095615_add_workspace_soft_delete` добавляет 3 nullable-колонки + индекс в `workspaces`. Additive, без breaking changes — применяется автоматически через `prisma migrate deploy`.
