...
describe('Аудит: Регистрация событий — ИБ-требования', () => {
  ...
  describe('Аудит действий администраторов (Req логирование §2.3, ГОСТ УЗП.22)', () => {
    ...
    it('изменение настроек системы создаёт AuditLog с action=admin.config.change', async () => {
  ...
  describe('Обязательные поля SIEM-события (Req логирование §1)', () => {
    it('каждый AuditLog-event содержит обязательные SIEM-поля', async () => {
    ...
    it('createdAt в AuditLog соответствует формату ISO-8601', async () => {
  ...
  describe('SIEM-тегирование (Req логирование §4)', () => {
    it('тег auth.login содержит ["flowtasks","auth","iia",<env>,<dc>]', async () => {
  ...
  describe('Маскировка ПДн в SIEM-событиях (Req логирование §2.4)', () => {
    it('email пользователя в AuditLog.meta маскируется перед отправкой в SIEM', async () => {
  ...
  describe('Системные события (Req логирование §2.6, ГОСТ ЦЗИ.30)', () => {
    it('health endpoint возвращает 200 (system.service.start эмитируется в server.ts при старте)', async () => {
  ...
  describe('Ошибки валидации логируются (Req логирование §2.6)', () => {
    it('POST /api/auth/login с невалидным телом создаёт событие system.validation.error', async () => {
